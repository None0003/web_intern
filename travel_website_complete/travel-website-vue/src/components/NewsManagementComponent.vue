    ...
    <div class="px-5 news-management-box">
        ...
        <ul class="news-list">
            <li class="row py-2 d-flex align-items-center bg-white content-box" v-for="post in posts" :key="post.id">
                ...
                <div class="col-md-2 create-date">{{ post.created_at }}</div>
                <div class="col-md-2 status">{{ formatStatus(post.status) }}</div>
    ...
    export default {
        ...
        setup() {
            ...
            const getPosts = async () => {
                try {
                    const response = await axios.get(`posts?page=${currentPage.value}&per_page=5`);
                    
                    if (Array.isArray(response.data)) {
                        posts.value = response.data;
                        totalPages.value = 1; // Update as needed based on API response
