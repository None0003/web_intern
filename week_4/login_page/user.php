...
class user {
    ...
    public function register($username, $email, $password) {
        ...
        $pass_notify = $this->validatePassword($password);
        ...
        if (empty($username_notify) && empty($email_notify) && empty($pass_notify)) {
            if ($this->isUsernameExists($username)) {
                ...
                $stmt = $this->connect->prepare("INSERT INTO user (email, password, username) VALUES (?, ?, ?)");
    ...
    public function login($username, $password) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            if (password_verify($password, $hashed_password)) {
                ...
                return "<span style='color:red;'>Invalid password.</span>";
            }
        } else {
            return "<span style='color:red;'>Username not found.</span>";
        }
    }
    ...
    private function validatePassword($password) {
        ...
        if (!preg_match('/[0-9]/', $password)) {
            return "<span style='color:red;'>Password must have at least a number.</span>";
        }
